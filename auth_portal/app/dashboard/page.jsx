...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      // Check if results already in session
      const cached = sessionStorage.getItem('rfp_results');
      if (cached) { router.push('/results'); return; }

      // Load vendor profile from user_information table
      ...
      if (error || !userInfo) {
        // No profile found — send to signup to complete profile
        router.push('/signup');
        return;
      }

      // Build profile object for recommender
      const profile = {
        legal_name: userInfo.company_name || '',
        website: userInfo.website || '',
        contact_name: userInfo.contact_name || '',
        contact_email: userInfo.contact_email || '',
        contact_phone: userInfo.contact_phone || '',
        hq_location: userInfo.hq_location || '',
        service_areas: userInfo.service_areas || '',
        years_in_business: userInfo.years_in_business || 0,
        uei: userInfo.uei || '',
        cage: userInfo.cage || '',
        naics_codes: userInfo.naics_codes
          ? userInfo.naics_codes.split(',').map(s => s.trim()).filter(Boolean)
          : [],
        certifications: userInfo.certifications
          ? userInfo.certifications.split(',').map(s => s.trim()).filter(Boolean)
          : [],
        company_description: userInfo.company_description || '',
        core_competencies: userInfo.core_competencies
          ? userInfo.core_competencies.split(',').map(s => s.trim()).filter(Boolean)
          : [],
        differentiators: userInfo.differentiators
          ? userInfo.differentiators.split(',').map(s => s.trim()).filter(Boolean)
          : [],
        selected_tags: userInfo.tags || [],
        past_performance: userInfo.past_performance || [],
        location_keywords: userInfo.location_keywords || '',
        open_only: userInfo.open_only ?? true,
        max_age_days: userInfo.max_age_days || 60,
        top_k: userInfo.top_k || 10,
        portfolio_pdf_name: userInfo.portfolio_pdf_name || '',
        portfolio_pdf_text: userInfo.portfolio_pdf_text || '',
      };
      ...
      try {
        ...
        if (res.ok) {
          ...
          sessionStorage.setItem('rfp_results', JSON.stringify(result.results));
        }
      } catch (e) {
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: '13px', color: 'var(--text-muted)' }}>Finding your opportunities…</p>
    </div>
  );
}
